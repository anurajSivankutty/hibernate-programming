...
public class HibernateApp {   
	...
	public static void saveEmployeeData(){
		...
		TypedQuery<Employee> query = session.createQuery("select e from Employee e where e.firstName=:name and e.payPerHour=:payPerHr",Employee.class);
		query.setParameter("name","Nipo");
		query.setParameter("payPerHr", 15F);
		List<Employee> employeeList = query.getResultList();
		
		if(!employeeList.isEmpty()) {
			System.out.println("Employee with payPerHour = 15  & Name = Nipo | Result size : " + employeeList.size() );
		...
		TypedQuery<Employee> nameQuery = session.createQuery("select e from Employee e where e.firstName=:name",Employee.class);
		nameQuery.setParameter("name","Nipo");
		List<Employee> employeeList2 = nameQuery.getResultList();
		
		if(!employeeList.isEmpty()) {
			System.out.println("Employee with name=Nipo  | Result size : " + employeeList2.size() );
